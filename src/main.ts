  ...
  <main class="container mx-auto mt-8 px-4">
    ...
    <section class="mb-8 rounded-lg bg-slate-100 p-6 shadow-lg">
      ...
      <div id="manualOcrOutput" class="mt-6">
            ...
            <p><strong>Routing Number:</strong> <span id="tesseractRoutingNumber">N/A</span></p>
            <p><strong>Account Number:</strong> <span id="tesseractAccountNumber">N/A</span></p>
            <p><strong>Check Number:</strong> <span id="tesseractCheckNumber">N/A</span></p>
            <h3 class="mt-4 text-xl font-semibold">OpenCV Results</h3>
            <p><strong>Routing Number:</strong> <span id="opencvRoutingNumber">N/A</span></p>
            <p><strong>Account Number:</strong> <span id="opencvAccountNumber">N/A</span></p>
            <p><strong>Check Number:</strong> <span id="opencvCheckNumber">N/A</span></p>
          </div>
    </section>
...
function displayCheckDetails(scanResults: ScanResults) {
    document.getElementById('routingNumber') !.textContent = scanResults.routingNumber;
    document.getElementById('accountNumber') !.textContent = scanResults.accountNumber;
    document.getElementById('checkNumber') !.textContent = scanResults.checkNumber;
}
...
function attachProcessImageListener() {
    if (currentProcessImageListener) {
        processImageButton.removeEventListener('click', currentProcessImageListener);
    }
        ...
        if (usingUserImage) {
            ...
            img.onload = async function() {
                ...
                try {
                    ...
                    displayManualCheckDetails(scanResults);
                } catch (error) {
                    console.error('Error processing check details:', error);
                }
            };
